...
def get_contract_df(contracts):
    ...
    month_cols = month_cols.reindex(
        sorted(month_cols.columns, key=lambda c: pd.to_datetime(c, format="%Y-%m")),
        axis=1
    )
...
hidden_columns = ['job_id', 'start_date', 'end_date', 'estimated_value', 'contract_hours', 'remains', 'contract_type', 'status', 'notes', 'created_at', 'allocations']
