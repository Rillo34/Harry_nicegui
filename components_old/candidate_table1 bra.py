...
class CandidateTable:
    ...
    def _build_ui(self):
                        ...
                        <q-menu>
                            <q-list style="min-width: 150px">
                                <q-item clickable v-close-popup
                                        @click="$emit('menu_action', {action: 'details', row: props.row})">
                                    <q-item-section>Visa detaljer</q-item-section>
                                </q-item>
                                <q-item clickable v-close-popup
                                        @click="$emit('menu_action', {action: 'edit', row: props.row})">
                                    <q-item-section>Redigera</q-item-section>
                                </q-item>
                                <q-item clickable v-close-popup
                                        @click="$emit('menu_action', {action: 'delete', row: props.row})">
        ...
        self.table.on('menu_action', lambda e: self._on_action(e.args))
    ...
    def _on_action(self, payload):
        ...
        print ("action")
        row = payload.get('row', {})
        print(f"[menu_action] {action} on {row.get('candidate_id')}")
        if action == 'details':
            ui.notify(f"Visa detaljer för {row.get('name')}")
        elif action == 'edit':
            ui.notify(f"Redigera {row.get('name')}")
        elif action == 'delete':
            ui.notify(f"Ta bort {row.get('name')}")
